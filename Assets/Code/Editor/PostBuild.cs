...
public class BuildPostprocessor
{
    ...
    static void CopyBugSplat(string builtProjectDir, string bugSplatDir)
    {
        var bugsplatFileNames = new List<string>()
        {
            "BsSndRpt.exe",
        ...
        foreach (var bugSplatFileName in bugsplatFileNames)
        {
            var srcFilePath = Path.Combine(bugSplatDir, bugSplatFileName);
            var dstFilePath = Path.Combine(builtProjectDir, bugSplatFileName);
